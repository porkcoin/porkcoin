...
void PorkSendPage::updateProgressBar()
{
     if(counter > 0)
     {
        counter--;
...
//将图片随机存入地图数组
void PorkSendPage::savePictoMap(void)
{
    int i,j;
...
//根据地图数组将容器中对应图片画出来
void PorkSendPage::DrawGameMap(QPainter *p)
{
    for(int i = 1; i < map_row - 1; i++)
    {
        for(int j = 1; j < map_col - 1; j++)
...
void PorkSendPage::paintEvent(QPaintEvent *)
{
    //窗口事件
    QPainter p(this);
...
void PorkSendPage::mousePressEvent(QMouseEvent *e)
{
    if(!isStart )return;
    int x,y;
    y = (e->x() / PHWIDTH);
...
bool PorkSendPage::isWin(int x0,int x1,int y0, int y1)
{
    lines.clear();
    bool ret;
    if(x0 == x1)
...
bool PorkSendPage::oneHLine(int x,int y0, int y1)
{
    int i;
...
bool PorkSendPage::oneVLine(int y, int x0, int x1)
{
    int i;
...
bool PorkSendPage::twoLine(int x0, int x1, int y0, int y1,bool ret)
{
...
/* 在两个给定点之间画一条较粗的直线 */
void PorkSendPage::drawLine(const int &aX, const int &aY,  const int &bX, const int &bY)
{
